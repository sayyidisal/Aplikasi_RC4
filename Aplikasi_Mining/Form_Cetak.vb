...
Public Class Form_Cetak
    ...
    Sub tampil_data()
        Using Koneksi As New OleDbConnection(My.Settings.Setting_Kon)
            Using Cmd As New OleDbCommand("Select * From tabel_dekripsi order by noIDsiswa", Koneksi)
                Koneksi.Open()
    ...
    Private Sub TextBox1_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles TextBox1.KeyDown
        If e.KeyCode = 13 Then
            Using Koneksi As New OleDbConnection(My.Settings.Setting_Kon)
                Using Cmd As New OleDbCommand("Select * From tabel_enkripsi where noIDsiswa='" & TextBox1.Text & "'", Koneksi)
                    Koneksi.Open()
                    Using Data As OleDbDataReader = Cmd.ExecuteReader
                        Data.Read()
    ...
    Private Sub Button2_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
        ...
        Try
            Using Koneksi As New OleDbConnection(My.Settings.Setting_Kon)
                Using Cmd As New OleDbCommand("Insert Into tabel_dekripsi Values('" & TextBox1.Text & _
                                              "','" & TextBox2.Text & "','" & ComboBox1.Text & _
                                              "','" & TextBox3.Text & "','" & ComboBox2.Text & "', '" & TextBox4.Text & "','" & TextBox5.Text & "')", Koneksi)
                    Koneksi.Open()
                    Cmd.CommandType = CommandType.Text
        ...
        If (Msg = DialogResult.Yes) Then
            Try
                Using conn As New OleDbConnection(My.Settings.Setting_Kon)
                    conn.Open()
                    Using cmd = New OleDbCommand("Delete From tabel_dekripsi Where noIDsiswa='" & TextBox1.Text & "'", conn)
